...
%% Load Data
% Load upsampled generalized tip stimulus from the tip base stimulus
% transform (sampling rate is 40 kHz)
load('Motor Tip Displacement.mat')
UpSample_WN_Stim=Tip_Signal.';

% Load generalized tip stimulus that was not upsamples (1kHz sampling rate)
WN_Stim=downsample(Tip_Signal.',40);

% Load Spike Trains and Identity of Units that are non-localized
load('Wing_NonBase_Identity_Spike_Trains.mat')
% NOTE: BASE in this file refers to NON-LOCALIZED UNITS
Coh_Base_Units=[25,27,28,29,30,31,32,...
    33,34,35,42,44,48,55,56,59,61]; % For ease of analysis, base also refers to nonbase
%[5,6,7,8,9,10,12,13,15,16,17,18,19,20,21,22,24,25,27,28,29,30,31,32,...
   % 33,34,35,42,44,48,55,56,59,61];
Num_Base_Neurons=length(Base_Unit_Store(1,1:end));
%% Run Encoding Property Analysis Iteratively for each unit
for neuron=Coh_Base_Units
    Identity=Base_Unit_Store{1,neuron}; % Name of neuron (Moth #, Unit #)
    ...
    % Load Spike Train of Units that have significant SR
    % Coherence
    ...
    Num_Repeats=30; % number of whitenoise repeats
    Duration_of_Repeat=10; % duration of whitenoise repeats in seconds
    ...
    Delay_Btwn_WN_Seg=3*FS; % Time delay between whitenoise segments in seconds
    Neuron=1;% Unit analysis selection (Number of units to analyze at a time)
    
    %% Plot Raster for each neuron over white noise repeats
    Raster_Time=linspace(0,length(WN_Stim)/1000,...
        length(UpSample_WN_Stim));
    Raster_Stim=2*UpSample_WN_Stim+39; % Tip stim
    ...
    FigPath='/Users/brandonpratt/Dropbox/Brandon/Moth Wing Paper/WingPaper_WorkingVersion/Code for Paper/WN Stimulus Analysis Code/NonBase Unit Encoding Figs';
    ...
    for Repeat=1:Num_Repeats
        Ind_Spikes=find(WN_Repeat_Matrix(1:end,Repeat)==1);
        Ind_Spikes=Ind_Spikes(find(Ind_Spikes>1600 & Ind_Spikes<length(UpSample_WN_Stim))); % Identify spike location within spike train data structure
        for j=1:length(Ind_Spikes)
            Spike_Count=Spike_Count+1;
            STS=UpSample_WN_Stim((Ind_Spikes(j)-Sample_Window):Ind_Spikes(j)); %Stimulus preceeding the spike
            STE(Spike_Count,1:Total_Sample_Window)=STS.';
            clearvars STS
        end
    ...
    save([ResultPath,'/NonBase Unit Encoding Results/STA and Std',Identity,'.mat'],'STA','Std_Error','-mat')
    save([ResultPath,'/NonBase Unit Encoding Results/STE',Identity,'.mat'],'STE','-mat')
    ...
    Num_Perms=40;
    % Randomly permute the spike train indices to construct the prior
    ...
    save([ResultPath,'/NonBase Unit Encoding Results/PSE',Identity,'.mat'],'Prior_Ensemble','-mat')
    ...
    % Save Permutated STE for Prior Construction
    save([ResultPath,'/NonBase Unit Encoding Results/Prior and Prior Std',Identity,'.mat'],'Prior','Prior_Std_Error','-mat')
    ...
    % Determine the the domiant eigenvalues and eigenvectors
    ...
    save([ResultPath,'/NonBase Unit Encoding Results/E1 and E2',Identity,'.mat'],'E1','E2','E1_eigenvector','E2_eigenvector','-mat')
    ...
    % Details of data structure above 
    % Determine if E1 and/or E2 from the STE is significant as compared to the eigenvalues derived from the PSE 
    cov_STE=cov(STE_Store{Neuron});
    cov_PRIOR=cov(Prior_Ensemble{Neuron});
