...
%     for a single neuron
% THIS PROCEEDING ANALYSIS WAS DONE ON NON_LOCALIZED UNITS
% Created by Brandon Pratt 
% Date: August 2016
clear all; close all; clc;
%% Load Data
% Stimulus Data
load('Motor Tip Displacement.mat') % Reconstructed Base Stimulus (after upsampling)
UpSample_WN_Stim=Tip_Signal.';
%Spike Trains
load('Wing_NonBase_Identity_Spike_Trains.mat')
Num_Neurons=length(Base_Unit_Store(1,1:end)); % BASE_UNIT_STORE was generically used for both base and non-locaized units. In this code it refers to non-localized units
%% Run Analysis for all non base idenitfied neurons
for neuron=51:64 %Loop for permutating all neurons %Adjust this to run less units
...
%% SR Coherence (stimulus response coherence)
figure, hold on
...
for j=1:Num_Repeats % 30 repeats of white noise
    for k=1:Num_Perms
    indice=k+ind(j);
...
%Identity: moth and neuron string
...
save([ResultsPath,'\Non Base Coherence Results\Sorted Reshape Perm SR Coherence neuron ',Identity,'.mat'],'Sorted_Reshape_Perm_SR_Coherence','-mat')
...
savefig(figure(10),[FigPath,'\Non Base Coherence Figs\SR Coh ',Identity,'.fig'])
%% Save Permutated Mean, std, and 95% confidence interval
%Identity: moth and unit number
save([ResultsPath,'\Non Base Coherence Results\Perm Mean, std, 95% Confidence, Frequencies of Sig Coh, Significant_Coherence_Values, and Neuron ',Identity,'.mat'],...
...
save(([ResultsPath,'\Non Base Coherence Results\Spike Rate, Info Rate, and Info Per Spike neuron ',Identity,'.mat']),'Spike_Rate','Integrate_Total_Information_Function','Information_Per_Spike','-mat');
