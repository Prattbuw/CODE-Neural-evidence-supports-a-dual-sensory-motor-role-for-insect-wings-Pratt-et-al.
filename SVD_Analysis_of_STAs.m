%% SVD of Spike Triggered Averages and Priors of base-localized and non-localized units to extract dominant modes
...
% Isolate Singular Values and Modes 
...
Norm_Singular_Values_Base=Singular_Values_Base./sum(Singular_Values_Base);
Norm_Singular_Values_NonBase=Singular_Values_NonBase./sum(Singular_Values_NonBase);
...
% Modes of both Base and Non-localized Units
figure(4)
img_count=0;
for Mode=1:length(Dominant_Modes(1,:))
...
% Weighted Modes of both Base and Non-localized Units
figure(5)
img_count=0;
for Mode=1:length(Dominant_Modes(1,:))
    img_count=img_count+1;
    % Plot dominant modes for Base units
    subplot(length(Dominant_Modes(1,:)),2,img_count)
    plot(Time.',Dominant_Modes_Base(:,Mode).*Norm_Singular_Values_Base(Mode),'k','LineWidth',3)
    title(['Dominant Mode of Base Units ', num2str(Mode)])
    xlabel('Time(ms)')
    ylabel('Displacement(mm)')
    xlim([-40 0]);
    ...
    img_count=img_count+1;
    subplot(length(Dominant_Modes(1,:)),2,img_count)
    plot(Time.',Dominant_Modes_NonBase(:,Mode).*Norm_Singular_Values_NonBase(Mode),'k','LineWidth',3)
    title(['Dominant Mode of Nonlocalized Units ', num2str(Mode)])
    xlabel('Time(ms)')
    ylabel('Displacement(mm)')
    xlim([-40 0]);
